...
#[derive(Asset, Debug, Clone, PartialEq, Serialize, Deserialize, Default, Reflect)]
    ...
    fn bake(
        ...
        for track in self.tracks.values() {
            ...
            for frame_index in 0..frame_count {
                let frame_time = time_range.start
                    + AnimationTime::from_seconds(frame_index as f64 / config.frame_rate)?;
...
mod tests {
    ...
    fn test_animation_baking_with_derivatives() {
        ...
        let config = BakingConfig::new(10.0)
            .with_derivatives(Some(AnimationTime::from_seconds(1.0).unwrap()));
        let mut registry = InterpolationRegistry::default();
        println!("{:?}", config);
        println!("{:?}", animation);
