...
fn test_id_parsing() {
    ...
    // Verify track id is preserved
    let twist_track = animation
        .tracks
        .values()
        .find(|t| t.name == "twist_joint")
        .expect("track exists");
    assert_eq!(
        twist_track.id.to_string(),
        "d7a6b716-10b0-40bb-a894-8bc13a992737"
    );

    // Verify keypoint id is preserved
    assert_eq!(
        twist_track.keypoints[0].id.to_string(),
        "061b069c-679c-4c8b-aeca-536de785b6bb"
    );
}