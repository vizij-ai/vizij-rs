...
use animation_player::value::{Value, Vector3, Vector4, Transform};
...
fn test_catmull_rom_basic_interpolation() {
    ...
    let k1 = track.add_keypoint(AnimationKeypoint::new(
        AnimationTime::from_seconds(0.0).unwrap(),
        Value::Float(0.0),
    )).unwrap();
    let k2 = track.add_keypoint(AnimationKeypoint::new(
        AnimationTime::from_seconds(1.0).unwrap(),
        Value::Float(10.0),
    )).unwrap();
    let k3 = track.add_keypoint(AnimationKeypoint::new(
        AnimationTime::from_seconds(2.0).unwrap(),
        Value::Float(20.0),
    )).unwrap();
    let k4 = track.add_keypoint(AnimationKeypoint::new(
    ...
    let value = track.value_at_time(
        ...
        &mut registry,
        Some(&AnimationTransition::new(k2.id, k3.id, TransitionVariant::Catmullrom)),
        &animation_data,
    );
...
// #[test]
// fn test_b_spline_interpolation() {
//     // Test the B-spline helper function directly
...
//     // At t=0, B-spline should be influenced by all 4 control points
//     let result_t0 = b_spline_curve(p0, p1, p2, p3, 0.0);
//     // At t=1, similarly influenced by all 4 points
//     let result_t1 = b_spline_curve(p0, p1, p2, p3, 1.0);
    
//     // B-spline doesn't pass through control points
//     assert_ne!(result_t0, p1);
//     assert_ne!(result_t1, p2);
    
//     // Test with Vector3
//     let v0 = Vector3::new(0.0, 0.0, 0.0);
//     let v1 = Vector3::new(10.0, 5.0, 0.0);
//     let v2 = Vector3::new(20.0, 5.0, 0.0);
//     let v3 = Vector3::new(30.0, 0.0, 0.0);
    
//     let result_v = b_spline_curve(v0, v1, v2, v3, 0.5);
//     // Should be somewhere in the middle
//     assert!(result_v.x > 5.0 && result_v.x < 25.0);
// }
...
fn test_spline_with_transform_values() {
    ...
    if let Some(Value::Transform(t)) = value {
        ...
        let rot_len = (t.rotation.x * t.rotation.x + 
                      ...
                      t.rotation.w * t.rotation.w).sqrt();
...
fn test_edge_cases_insufficient_keypoints() {
    ...
    let value = track.value_at_time(
        AnimationTime::from_seconds(0.5).unwrap(),
        &mut registry,
        Some(&AnimationTransition::new(k1.id, k2.id, TransitionVariant::Catmullrom)),
        &animation_data,
    );
...
fn test_interpolation_context_get_point() {
    let keypoints = vec![
        ...
        AnimationKeypoint::new(AnimationTime::from_seconds(1.0).unwrap(), Value::Float(10.0)),
        AnimationKeypoint::new(AnimationTime::from_seconds(2.0).unwrap(), Value::Float(20.0)),
        AnimationKeypoint::new(AnimationTime::from_seconds(3.0).unwrap(), Value::Float(30.0)),
    ];
