 ...
 * many faces as it shows: each is a {@link Device} of its own, JS-paced,
...
/**
 * A task run's lifecycle contract, as {@link Device.spawn} resolves it: the
 ...
 * `stop` call {@link Device.halt} issues. Keys are store paths.
 */
...
/** A click on a face, by the ids its RobotData declares. */
 ...
 * plus whether the bundle's neutral pose is staged (default `true`), and the
 * speech: `audio` is the playback hook the `say` provider hands its audio
 * to — the page's Web Audio player by default, `false` for a device that
 * plays no speech — and `speechApiUrl` the TTS deployment. */
...
interface WasmBindings {
  ...
  FaceDevice: {
    fromGraph(graph_json?: string): WasmFaceDevice;
  };
 ...
 */
export class Device {
  private inner: WasmFaceDevice;

  constructor(inner: WasmFaceDevice) {
    this.inner = inner;
  }
...
/** @deprecated Renamed {@link Device}: a face's device, or one with no face. */
export type Runtime = Device;
...
export async function loadFace(
  ...
  return new Device(
    bindings().loadFace(faceId, bytes, options ? JSON.stringify(rest) : undefined, play),
  );
}

/** Take the face down: its scene, its camera, its GLB. Dispose its
 * {@link Device} afterwards. */
...
/** The clicks on faces since the last drain, oldest first. */
 ...
 */
export async function startRuntime(graph?: GraphSpecInput, input?: InitInput): Promise<Device> {
  await init(input);
  const graphJson =
    graph === undefined ? undefined : typeof graph === "string" ? graph : JSON.stringify(graph);
  return new Device(bindings().FaceDevice.fromGraph(graphJson));
}
