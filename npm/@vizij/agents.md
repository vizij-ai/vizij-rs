...
- **Packages**: `@vizij/animation`, `@vizij/node-graph`, `@vizij/orchestrator-wasm`, `@vizij/value-json`, `@vizij/test-fixtures`, `@vizij/wasm-loader`.
- **Bootstrap**: `pnpm install` from repo root; rebuild shared packages with `pnpm run build:shared`.
- **Local links for `vizij-web`**: Build the stacks you need, then in `vizij-web` run `pnpm run wasm:link` (or scope with `WASM_PKGS="node-graph orchestrator-wasm"`). `pnpm run wasm:status` confirms whether the workspace points at `vizij-rs` or the registry. Use `pnpm run wasm:unlink` when you’re done.
