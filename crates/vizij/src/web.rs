//! The browser module: the view and the device in one wasm module, one App
...
//! device of its own ([`FaceDevice`]), JS-paced (the page calls `step(dt)`
...
/// The clicks on faces since the last drain, oldest first, as
...
fn describe_json(meta: &FaceMeta) -> serde_json::Value {
    let feature = |feature: &FeatureKind| match feature {
        FeatureKind::Translation => "translation".to_string(),
        FeatureKind::Rotation => "rotation".to_string(),
        FeatureKind::Scale => "scale".to_string(),
        FeatureKind::Color => "color".to_string(),
        FeatureKind::Opacity => "opacity".to_string(),
        FeatureKind::Morph(target) => target.clone(),
    };
    let mut animatables: Vec<(String, serde_json::Value)> = meta
        ...
        .map(|(uuid, binding)| {
            (
                uuid.clone(),
                serde_json::json!({ "node": binding.node_name, "feature": feature(&binding.feature) }),
            )
        })
        .collect();
...
impl FaceDevice {
    ...
    pub fn from_graph(graph_json: Option<String>) -> Result<FaceDevice, JsValue> {
...
fn parse_view_options(json: Option<&str>) -> Result<ViewOptions, JsValue> {
        ...
        .and_then(serde_json::Value::as_str)
    {
        Some(hex) => {
            let [r, g, b] =
                device::parse_rgb(hex).map_err(|e| JsValue::from_str(&e.to_string()))?;
            Color::srgb_u8(r, g, b)
        }
        None => Color::NONE,
    };
...
/// spec is ready for [`FaceDevice::from_graph`] or
/// [`loadGraph`](FaceDevice::load_graph), so an exported GLB can be deployed
...
///   built by [`load_face`] or [`FaceDevice::from_graph`] always links the
