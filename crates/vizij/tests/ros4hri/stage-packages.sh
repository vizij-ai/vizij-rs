# Stage the ROS4HRI skill interface packages — `std_skills`,
...
#
# Sourced, not run: `source stage-packages.sh; stage_ros4hri_packages <src-dir> <repo-root>`.
...
')"
  [ -d "$MSGS/std_skills" ] && [ -d "$MSGS/communication_skills" ] && [ -d "$MSGS/interaction_skills" ] && [ -d "$MSGS/hri_msgs" ] \
    || { echo "no ROS4HRI skill definitions under $MSGS" >&2; return 1; }
