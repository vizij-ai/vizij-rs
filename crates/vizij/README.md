...
| `device::bridge` | the open local bridge: the WebSocket server with the face's inputs and skills in its registry, the control panel on the same port | every target but the browser |
