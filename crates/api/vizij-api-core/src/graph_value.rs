//! The value abstraction the node-graph evaluator is generic over.
//!
...
//! as a trait so evaluators can be parameterized over any value type that
//! implements it. This trait lives in `vizij-api-core` alongside the vocabulary
//! it mirrors; the built-in impl for [`Value`] simply delegates to the existing
//! free functions, so graph-core carries no concrete value semantics.
...
pub use crate::value::{Transform, VizijKind};

use crate::value as vocab;
use crate::{coercion, Value};
...
/// `vizij-api-core` and are *not* the value type itself.
