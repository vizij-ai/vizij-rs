    ...
    pub fn set_param(&mut self, node_id: &str, key: &str, json_value: &str) -> Result<(), JsValue> {
        ...
        if let Some(node) = self.spec.nodes.iter_mut().find(|n| n.id == node_id) {
            match key {
                ...
                "in_min" => node.params.in_min = Some(if let Value::Float(f)=val { f } else { 0.0 }),
                "in_max" => node.params.in_max = Some(if let Value::Float(f)=val { f } else { 0.0 }),
                "out_min" => node.params.out_min = Some(if let Value::Float(f)=val { f } else { 0.0 }),
                "out_max" => node.params.out_max = Some(if let Value::Float(f)=val { f } else { 0.0 }),
