...
impl CollectionKind {
    /// Return the number of accumulated channels in the collection.
    fn len(&self) -> usize {
        match *self {
    ...
    /// Check whether two collection kinds are compatible for blending.
...
/// Convert a numeric value into a collection kind + scalar list.
...
impl AccumEntry {
    ...
    /// Collapse the accumulator into a final blended value, if any.
