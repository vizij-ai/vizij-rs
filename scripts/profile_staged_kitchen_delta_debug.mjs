#!/usr/bin/env node
/**
 * Staged kitchen benchmark (debug build) exercising ONLY the new slots+delta
 * API. Run the legacy script separately for before/after comparisons and use
 * this one for flamegraphs focused on the improved path. Uses the nameful
 * debug wasm build so CPU profiles remain readable.
 *
 * Env overrides:
...
function slotsDeltaSample() {
  const { g } = makeGraph(SIZE);
  const { idx, paths } = registerInputs(g, SIZE);
  const vals = new Float32Array(paths.length);

  // Warm once to build plan/cache and establish baseline version.
  vals.fill(0);
  g.stage_inputs_slots(idx, vals);
  g.set_time(0);
  g.step(DT);
  g.eval_all_slots();
  let lastVersionObj = g.get_outputs_full();
  let lastVersion = BigInt(
    (lastVersionObj && typeof lastVersionObj.version !== "undefined"
      ? lastVersionObj.version
      : 0) ?? 0,
  );
  ...
  for (let s = 0; s < STEPS; s += 1) {
    ...
    g.set_time(s * DT);
    g.step(DT);
    // Eval without JSON, then pull delta to include serialization cost.
    const ver = g.eval_all_slots();
    const delta = g.get_outputs_delta(lastVersion);
    lastVersion =
      (delta && typeof delta.version !== "undefined"
        ? BigInt(delta.version)
        : BigInt(ver ?? lastVersion));
  }
...
function computeSignature() {
  ...
  g.step(DT);
  g.eval_all_slots();
  const outObj = g.get_outputs_full();
  return signature(outObj);
}

function runSamples(fn) {
  const samples = [];
...
function main() {
  ...
  const slotsDelta = runSamples(slotsDeltaSample);
  console.log(
    `staged_kitchen_debug_slots_delta SIZE=${SIZE} STEPS=${STEPS} SAMPLES=${SAMPLES}`,
  );
  console.log(`  slots_delta_per_step_us=${slotsDelta.toFixed(3)}`);
